...
// Supervisor CET is not fully virtualized yet. The compatibility path hides
// CET from the guest and intercepts its user and supervisor MSRs.
...
// Production mode launches the same validated VMX contract on every
// participating logical processor. Set this only in a dedicated bring-up
// build when a debugger must isolate one processor deliberately.
static constexpr bool kDebugSingleCpu = false;
static constexpr u32  kDebugCpuIndex = 0;

// keep the processor that owns the startup rendezvous native. on a late load
// from a live kernel-debug session this is also the processor most likely to
// carry the debugger transport interrupt. HyperDbg uses a single broadcast
// barrier, but a failed callback can strand that barrier without a diagnostic
// continuation, so the coordinator is excluded until VM-exit is proven here
...
static __forceinline u32 ExpectedLaunchProcessorCount() {
    ...
    if (g_ProcessorCount == 0) return 0;
    return g_ProcessorCount - (kReserveCoordinatorCpu ? 1U : 0U);
}
...
static constexpr u32 kHvCrashBlobVersion = 10;
...
static __forceinline u64 GetCr4GuestHostMask() {
    // Bits in the guest-host mask are supplied by CR4_READ_SHADOW on MOV
    // from CR4. Keep the mask limited to state that this monitor can enforce.
    return CR4_VMXE |
...
static __forceinline bool IsValidGuestState(const GuestContext* c) {
    ...
    // The restore path writes a three-word ring-0 IRETQ frame and its private
    // spill below RSP. Reject values that would underflow that frame or point
    ...
    if (msrIndex == MSR_IA32_XSS) {
        if (!g_XsavesEnabled) {
            ...
            if (g_VcpuData && id < g_ProcessorCount) {
                g_VcpuData[id].GuestXss = 0;
            }
            return true;
        }
        // IA32_XSS is not a VMCS field. Keep a per-vcpu shadow and restore it
        // after XRSTORS so a legal context-switch write is not retried forever
...
static bool ConfigureMsrBitmap(VcpuContext* vcpu) {
    ...
    // A one bit means "cause a VM-exit".  VMX loads these values from the
    // guest VMCS on every entry and restores the host copies on every exit.
    // A native guest WRMSR would update only the hardware MSR, leaving the
    // VMCS stale; the next transition would then restore the old value. This
    // is especially dangerous for Windows FS/GS and SYSENTER context state.
    // Keep the software shadow and VMCS synchronized through the same handler
    // path used by HyperDbg.
    ...
    // trap the complete RTIT window and leave VMX before native PT access
    for (u32 msr = MSR_IA32_RTIT_OUTPUT_BASE; msr <= 0x58FU; ++msr) {
        if (!setBit(msr, false) || !setBit(msr, true)) return false;
    }

    // IA32_XSS has no VMCS field, so keep the initial selector per virtual CPU
    // and leave VMX before a later selector or CET contract change
    if (!setBit(MSR_IA32_XSS, false) || !setBit(MSR_IA32_XSS, true)) {
        return false;
    }
    // The guest contract hides user CET even when the host XSAVES frame can
    // preserve its component: the MSR handlers expose a reset read view and
    // reject writes, so native non-root access must never reach the hardware.
    // Trap these MSRs unconditionally to prevent a guest from changing the
    // logical processor's CET state behind the VMCS contract.
    constexpr u32 hiddenCetUserMsrs[] = {
        MSR_IA32_U_CET, MSR_IA32_PL3_SSP,
    };
    for (u32 msr : hiddenCetUserMsrs) {
        if (!setBit(msr, false) || !setBit(msr, true)) return false;
    }
            ...
            if (firstExitProbeResponse) {
                ...
                if ((basicLeaf || extendedLeaf) && !hypervisorLeaf) {
                    ...
                    if (subleafSupported && leaf == 1) {
                        ...
                        // D.1 ECX/EDX describe components that the guest may
                        // select, while EBX describes the current selection.
                        // Keep those two views separate because the fixed
                        // XSAVES frame preserves the interrupted host XSS.
                        const u64 guestXssCapabilityMask =
                            g_XsavesEnabled ? g_SupportedXssMask : 0ULL;
                        const u64 guestXssSelection =
                            g_XsavesEnabled
                                ? (vcpu->GuestXss & guestXssCapabilityMask)
                                : 0ULL;
                        ...
                        // IA32_XSS is a guest selector. ECX/EDX are capability
                        // masks, not the current selector, so preserve the
                        // architectural component descriptions while hiding
                        // state that this monitor cannot restore.
                        regs[2] &= static_cast<int>(
                            static_cast<u32>(guestXssCapabilityMask));
                        regs[3] &= static_cast<int>(
                            static_cast<u32>(guestXssCapabilityMask >> 32));
                    } else if (subleafSupported && leaf == 0xD &&
                        ...
                        const bool xssComponent =
                            (guestXssCapabilityMask & component) != 0;
...
bool SetupVmcs(const VcpuContext* Vcpu, void* GuestSp, void* GuestIp) {
    ...
    if (!IsCanonical(gdtBase) || !IsCanonical(idtBase) ||
        ...
        !IsValidArchitecturalCr3(guestCr3, guestCr4) ||
        tssBase == 0) {
    ...
    if (!GuestIp || !GuestSp ||
        ...
        (reinterpret_cast<u64>(GuestSp) & 0xFULL) != 0) {
    ...
    // keep interrupts masked while the launch frame is the active guest stack
    // the thunk restores the saved flags only after it has moved back to the
    // caller stack, which closes the late-launch corruption window
    u64 guestRflags = GetRflags();
    const u64 callbackRflags = guestRflags;
    guestRflags |= (1ULL << 1);                 // architectural fixed bit
    guestRflags &= ~((1ULL << 9) |              // IF during stack handoff
                     (1ULL << 17) |             // VM (invalid in long mode)
                     (1ULL << 19) |             // VIF
                     (1ULL << 20));             // VIP
    HV_VERBOSE_PRINT("[HV] CPU %u guest launch flags: source=0x%llX "
                     "handoff=0x%llX source_if=%u handoff_if=%u irql=%u "
    ...
    const u64 cr4GuestHostMask = GetCr4GuestHostMask();
    // MOV-from-CR4 returns unmasked bits from GUEST_CR4 and masked bits from
    // CR4_READ_SHADOW. Store the guest-visible values only in the masked view.
    const u64 cr4ReadShadow = guestCr4 & cr4GuestHostMask;
...
extern "C" ULONG PrepareHvCallback(ULONG_PTR Context, void* GuestSp, void* GuestIp) {
    ...
    __try {
        ...
        HV_VERBOSE_PRINT("[HV] CPU %u local contract: CR4=0x%llX XSAVES=%u "
                         "XSS=0x%llX CET_VMCS=%u branch=%s\n", id, localCr4,
                         g_XsavesEnabled ? 1U : 0U,
...
static VOID TargetLaunchDpcRoutine(PKDPC Dpc,
    ...
    if (priorState == TargetWorkQueued &&
        ...
        const bool firstExitProbePassed =
...
static bool LaunchResultNeedsDetail(const VcpuContext& vcpu) {
...
static void PrintLaunchResult(u32 processorIndex, const VcpuContext& vcpu) {
    ...
    const u64 launchCr3Metadata =
        ReadLaunchCr3Field(&vcpu.LaunchCr3Metadata);
    if (!LaunchResultNeedsDetail(vcpu)) {
    ...
    // retain the complete failure block for WinDbg and the crash blob
    HV_PASSIVE_PRINT(
        "[HV] CPU %u launch result: state=%ld stage=%ld check=%ld "
        "vmexits=%ld launch_flags=0x%llX raw_reason=0x%08X basic=%u "
        "msr=0x%08X msr_value=0x%llX "
        "entry_failure=%u probe=%ld probe_exits=%ld probe_resumes=%ld "
        "probe_reason=%ld probe_action=%ld probe_flags=0x%llX "
        "action=%ld resumes=%ld rip=0x%llX "
        "rsp=0x%llX exit_len=%llu qualification=0x%llX "
        "instrerr=0x%llX resume_flags=0x%llX setup_phase=%ld "
        "vmwrite_failed=%ld write_field=0x%llX write_flags=0x%llX "
        "write_error=0x%llX vmread_failed=%ld read_field=0x%llX "
        "read_flags=0x%llX read_error=0x%llX vmcs_mismatch=%ld "
        "mismatch_field=0x%llX mismatch_expected=0x%llX "
        "mismatch_actual=0x%llX mismatch_mask=0x%llX "
         "validity=0x%llX "
         "vmclear=0x%llX vmcs_current=%ld vmptrld=0x%llX "
         "primary_caps=0x%llX tertiary_caps=0x%llX "
        ...
        vcpu.LastExitReasonBasic, vcpu.LastExitMsrIndex,
        vcpu.LastExitMsrValue, vcpu.LastExitEntryFailure,
        firstExitProbeState, firstExitProbeExits, firstExitProbeResumes,
        InterlockedCompareExchange(
        ...
        action, vcpu.VmResumeAttempts, vcpu.LastGuestRip,
        vcpu.LastGuestRsp, vcpu.LastExitInstructionLength,
        vcpu.LastExitQualification, vcpu.LastVmInstructionError,
        vcpu.LastVmResumeFlags,
        vcpu.VmcsSetupPhase, vmcsWriteFailed, vcpu.FirstVmcsWriteField,
        vcpu.FirstVmcsWriteFlags, vcpu.FirstVmcsWriteError,
        vmcsReadFailed, vcpu.FirstVmcsReadField,
        vcpu.FirstVmcsReadFlags, vcpu.FirstVmcsReadError,
        vmcsValueMismatch, vcpu.FirstVmcsMismatchField,
        vcpu.FirstVmcsMismatchExpected, vcpu.FirstVmcsMismatchActual,
         vcpu.FirstVmcsMismatchMask, diagnosticValidity,
         vcpu.LastVmclearFlags,
         InterlockedCompareExchange(
             const_cast<volatile LONG*>(&vcpu.VmcsCurrent), 0, 0),
         vcpu.LastVmptrldFlags, vcpu.PrimaryControlsCapability,
         vcpu.TertiaryControlsAllowed, launchRawGuestCr3,
         launchGuestCr3, launchRawHostCr3, launchHostCr3,
         launchCr3Metadata);
}
...
extern "C" NTSTATUS StartHypervisor() {
    ...
    if constexpr (kDebugSingleCpu) {
        ...
        //
        // The single-CPU launch has completed. Use another native processor to
        // observe the first VMCALL-induced VM-exit without letting the target run
        // through the normal VM-exit prologue immediately.
        //

        HV_PASSIVE_PRINT(
            "[HV] DEBUG single-CPU launch succeeded; "
            "arming VM-exit stage probe\n");

        //
        // Once this flag is set, the next VM-exit on the debug CPU will stop at the
        // current hold point in HvVmExitEntryPoint. A frozen machine therefore means
        // execution reached that point. A reset means the processor failed before it.
        //
        InterlockedExchange(
            &g_HvVmExitDebugHold,
            1);
        ...
        //
        // Do not print anything after setting the hold flag. Kernel debugger output
        // can require interprocessor coordination, while the debug CPU deliberately
        // spins with interrupts disabled.
        //
        targetStatus =
            QueueTargetOperation(
                debugCpu,
                TargetOperationStop);
        ...
        if (!NT_SUCCESS(targetStatus)) {
            ...
            HV_PASSIVE_PRINT(
                "[HV] DEBUG failed to queue VM-exit stage probe: 0x%08X\n",
                static_cast<ULONG>(targetStatus));
        ...
        //
        // Reaching this point is not expected while the ASM hold is enabled.
        // The target processor should remain stopped at the selected VM-exit stage.
        //
        ReleaseCoordinatorAffinity(&coordinatorAffinity,
                                    &coordinatorBound);
        return STATUS_SUCCESS;
    ...
    if constexpr (kUseHyperDbgGenericLaunch) {
        ...
        // do not queue the reserved processor after the staged pass. It stays
        // native so the startup thread and debugger transport retain a normal
        // execution context while the transaction is published.
    }
