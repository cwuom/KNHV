...
For a stop/start regression, verify the image that actually loaded before interpreting the result. The first KD line must contain the current `V58-...-CET-PL0-PRESERVE-SEGMENT-VALIDATE...` contract tag; a `V57-...` or `V56-...` line means that the target service is still loading an older copy. The private `hv_probe.exe` is expected to report `0/28` after a clean unload because its response is produced only by the VMX non-root exit path. Run it again after the next successful start, where a healthy all-core launch should return `28/28`.
